...
from models.fusion import FeatureFusionModule as FFM
from models.fusion import FeatureRectifyModule as FRM
from models.TFAM import TFAM as Tfma
...
class BoundaryDetection(nn.Module):
    def __init__(self, in_channels=4, num_classes=4):
        ...
        self.conv2 = nn.Conv2d(64, num_classes * (num_classes - 1), kernel_size=3, padding=1)
...
class UltraLight_VM_UNet(nn.Module):
    ...
    def __init__(self, num_classes=4, input_channels=3, c_list=[8, 16, 24, 32, 48, 64],
        ...
        input = [8, 16, 24, 32, 48, 64]
        self.TT = nn.ModuleList([
            Tfma(in_channel=input[0]),
            Tfma(in_channel=input[1]),
            Tfma(in_channel=input[2]),
            Tfma(in_channel=input[3]),
            Tfma(in_channel=input[4]),
            Tfma(in_channel=input[5])])
    ...
    def forward(self, x, y=None):
        ...
        t1 = out  # b, c0, H/2, W/2  [32, 8, 256, 256]

        out = F.gelu(F.max_pool2d(self.ebn2(self.encoder2(out)), 2, 2))
        t2 = out  # b, c1, H/4, W/4  [32, 16, 128, 128]

        out = F.gelu(F.max_pool2d(self.ebn3(self.encoder3(out)), 2, 2))
        t3 = out  # b, c2, H/8, W/8  [32, 24, 64, 64]

        out = F.gelu(F.max_pool2d(self.ebn4(self.encoder4(out)), 2, 2))
        t4 = out  # b, c3, H/16, W/16  [32, 32, 32, 32]

        out = F.gelu(F.max_pool2d(self.ebn5(self.encoder5(out)), 2, 2))
        t5 = out  # b, c4, H/32, W/32  [32, 48, 16, 16]


        if y is not None:
            if y.size()[1] == 1:
                y = y.repeat(1, 3, 1, 1)
            out_y = F.gelu(F.max_pool2d(self.ebn1(self.encoder1(y)), 2, 2))
            y1 = out_y
            out_y = F.gelu(F.max_pool2d(self.ebn2(self.encoder2(out_y)), 2, 2))
            y2 = out_y
            out_y = F.gelu(F.max_pool2d(self.ebn3(self.encoder3(out_y)), 2, 2))
            y3 = out_y
        ...
        out = F.gelu(self.encoder6(out))  # b, c5, H/32, W/32  [32, 64, 16, 16]
        ...
        boundary_map = self.boundary_detection(out0)  # 边界检测模块的输出

        return out0, boundary_map
